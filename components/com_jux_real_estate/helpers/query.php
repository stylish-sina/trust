...
class JUX_Real_EstateHelperQuery extends JObject {
	...
	$query = $db->getQuery(true);
	$query 	->select('DISTINCT(t.id), t.id AS value, t.title AS text')
			->from('#__re_types as t')
			->join('LEFT', '#__languages as l ON t.language=l.lang_code')
			->where('t.published = 1');

	if(isset($language_tyle)){
		$query->where("t.language LIKE '%$language_tyle%' ");
	}

	if ($available) {
	    $query->join('LEFT', '#__re_realties AS r ON r.type_id = t.id');
	}
