...
class JUX_Real_EstateFields {
	...
	<div class="control-group">
	    ...
	    <div class="controls">
		<input type="text" id="<?php echo $row->name; ?>" name="<?php echo $row->name; ?>" class="<?php echo $row->css_class; ?> <?php if ($row->required) echo 'required'; ?>" <?php if ($row->required) echo 'required="required"'; ?> size="<?php echo $row->size; ?>" value="<?php echo JText::_($postedValue); ?>" required="required" />
	    </div>
	</div>
	<?php
    }
	...
	<label for="<?php echo $row->name; ?>"><?php echo JText::_($row->title); ?></label>
	<input type="text" size="18" name="<?php echo $row->name; ?>" class="input-medium" value="" />
	...
	<div class="control-group">
		...
		<input type="text" name="<?php echo $row->name; ?>" class="<?php echo $row->css_class; ?> <?php if ($row->required) echo 'required'; ?>" <?php if ($row->required) echo 'required="required"'; ?> size="<?php echo $row->size; ?>" value="<?php echo JText::_($value); ?>"  />
	    </div>
	</div>
	<?php
    }
	...
	<tr>
	    ...
	    <td>
		<textarea id="<?php echo $row->name; ?>" name="<?php echo $row->name; ?>" rows="<?php echo $row->rows; ?>" cols="<?php echo $row->cols; ?>" class="<?php echo $row->css_class; ?> <?php if ($row->required) echo 'required'; ?>" <?php if ($row->required) echo 'required="required"'; ?>><?php echo $postedValue; ?></textarea>
	    </td>
	</tr>
	<?php
    }
	...
	?>
	<label for="<?php echo $row->name; ?>"><?php echo JText::_($row->title); ?> 
	...
	<tr>
	    ...
	    <td>
		<textarea name="<?php echo $row->name; ?>" rows="<?php echo $row->rows; ?>" cols="<?php echo $row->cols; ?>" class="<?php echo $row->css_class; ?> <?php if ($row->required) echo 'required'; ?>" <?php if ($row->required) echo 'required="required"'; ?>><?php echo JText::_($value); ?></textarea>
	    </td>
	</tr>
	<?php
    }
	...
	<label for="<?php echo $row->name; ?>"><?php echo JText::_($row->title); ?></label>
	<?php
	echo JHTML::_('select.genericlist', $options, $row->name, ' class="input-medium" ', 'value', 'text');
	?>
...
//                var_dump($required);
	...
	$values = explode("\r\n", $row->values);
	$html = '<tr><td><label for="' . $row->name . '">' . JText::_($row->title) . '</lable>';
    ...
    /**
     * Render search checkbox list for horizontal
     *
     * @param object $row
     */
    function _renderSearchCheckboxListHorizontal($row) {
	$values = explode("\r\n", $row->values);
	$html = '<label for="' . $row->name . '">' . JText::_($row->title) . '</label>';

	for ($i = 0, $n = count($values); $i < $n; $i++) {
	    $value = $values[$i];
	    $html .= '<input class="checkbox" value="' . $value . '" type="checkbox" name="' . $row->name . '[]" ><lable for="' . $row->name . '[]">' . JText::_($value) . '</lable>';
	}
	...
	?>
	<tr>
	    <td class="key"><?php echo JText::_($row->title); ?>
	...
	for ($i = 0, $n = count($this->_searchfields); $i < $n; $i++) {
		...
		case FIELD_TYPE_HEADING :
		    ?>
		    <td colspan="6" class="heading"><?php echo JText::_($row->title); ?></td>
		    <?php
		    break;
		case FIELD_TYPE_MESSAGE :
		    ?>	
    ...
    /**
     * Render published custom search fields horizontal
     *
     */
    function renderCustomSearchFieldsHorizontal() {
	ob_start();
	for ($i = 0, $n = count($this->_searchfields); $i < $n; $i++) {
	    $row = $this->_searchfields[$i];
	    switch ($row->field_type) {
		...
		case FIELD_TYPE_CHECKBOXLIST :
		    $extraHtml = '<span>';
		    $extraHtml .= $this->_renderSearchCheckboxListHorizontal($row);
		    $extraHtml .= '</span>';
		    break;
		case FIELD_TYPE_RADIOLIST :
		    echo '<span>';
     ...
     * @param object $donorId
     */
    public static function getCustomFields($realtyId, $isOwner) {
	$db = JFactory::getDBO();
     ...
     *
     */
    function renderCustomSearchFields2() {
	ob_start();
	for ($i = 0, $n = count($this->_searchfields); $i < $n; $i++) {
	    $row = $this->_searchfields[$i];
	    if ($i % NUMBER_OPTION_PER_LINE == 0) {
		...
		case FIELD_TYPE_TEXTBOX :
		    echo '<tr><td>';
		    $this->_renderSearchTextBox2($row);
		    echo '</td></tr>';
		    break;
		case FIELD_TYPE_TEXTAREA :
		    echo '<tr><td>';
		...
		case FIELD_TYPE_DROPDOWN :
		    echo '<tr><td>';
		    $this->_renderSearchDropdown2($row);
		    echo '</td></tr>';
		    break;
		case FIELD_TYPE_CHECKBOXLIST :
		    $extraHtml = $this->_renderSearchCheckboxList($row);
    ...
    function _renderSearchTextBox2($row) {
	?>
	<div class="<?php echo $row->name; ?>-field">
	    <input type="text" size="18" name="<?php echo $row->name; ?>" class="input-medium" value="<?php if (isset($_REQUEST[$row->name]) && $_REQUEST[$row->name]) {
	    echo $_REQUEST[$row->name];
     ...
     * @param object $row
     */
    function _renderSearchDropdown2($row) {
	$options = array();
	$options[] = JHTML::_('select.option', '', JText::_($row->title));
	$values = explode("\r\n", $row->values);
	for ($i = 0, $n = count($values); $i < $n; $i++) {
	    $options[] = JHTML::_('select.option', $values[$i], JText::_($values[$i]));
	}
