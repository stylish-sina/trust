...
class JUX_Real_EstateModelMessage extends JModelList {
	...
	// xử lý lại phần  E-mail Body Agent back-end
	
	$body_agent1 = str_replace('{email}', $post['email'], $config_email->email_agent_to_user_body);// lối thêm email
	$body_agent2 = str_replace('{name}', $post['name'], $body_agent1);//lối thêm name
	$body_agent3 = str_replace('{content}', $post['content'], $body_agent2);//lối thêm name
	$body = str_replace('{toemail}', $post['toemail'], $body_agent3);//lối thêm toemail
	
	$mailer->setSubject($config_email->email_agent_to_user_subject);
	$mailer->isHTML(true);
	...
	// $body2 = str_replace('{email}', $post['toemail'], $config_email->email_user_to_agent_body);
	// xử lý lại phần  E-mail Body User send email back-end
	$body_user1 = str_replace('{name}', $post['name'], $config_email->email_user_to_agent_body);
	$body_user2 = str_replace('{email}', $post['email'], $body_user1);
	$body_user3 = str_replace('{user_phone}', $post['user_phone'], $body_user2);
	$body_user4 = str_replace('{content}', $post['content'], $body_user3);
	$body2 = str_replace('{toemail}', $post['toemail'], $body_user4);
	
	$mailer2->setSubject($config_email->email_user_to_agent_subject);
	$mailer2->isHTML(true);
